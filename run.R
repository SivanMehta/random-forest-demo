...
group.1 <- tibble(x = runif(n) * 10, y = f(x) + runif(n, -1, 1), group = 0) %>%
  filter(abs(y - f(x)) > .5)
group.2 <- tibble(x = runif(n/2) * 10, y = f(x) + runif(n/2, -.4, .4), group = 1)
...
accuracy <- round(length(which(predictions$group == predictions$group.hat)) / nrow(predictions) * 10000)
title <- paste(
  'Grey are correctly classified, red was wrong. Overall accuracy: ', accuracy / 100, '%',
  sep = ''
)
